...
local lexer = require'assets/codebase/libs/lua-lexer'
    ...
    drawCode = function(self)
      ...
      for y = 1, linesToDraw do
        ...
        if currentLine == self.cursor.y then
          --self.drawToken("|", self.cursor.x - self.scroll.x - 0.5, y * 28, 'caret')
         -- self.drawToken("|", game.font:getWidth(line:sub(1, self.cursor.x)) + 45, (y+1) * 28, 'caret')

          if terminal.cursor then         
            love.graphics.setColor(1,1,1)
            love.graphics.line(game.font:getWidth(line:sub(1, self.cursor.x)) + 45, (y+1) * 28, game.font:getWidth(line:sub(1, self.cursor.x)) + 45, (y+1) * 28 + 25)
          end
        end
        -- draw single line of text
        self.drawToken(currentLine, 4, (y+1)*28, "background")
        local lineTokens = self.lexed[currentLine]
        for j, token in ipairs(lineTokens) do
          self.drawToken(token.data, x, (y+1)*28, token.type)
          --print('token',x,y)
          --x = x + #token.data
          x = x + game.font:getWidth(token.data)
        end
      end
      -- status line
